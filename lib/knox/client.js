...
Client.prototype.beginUpload = function(filename, headers, fn){
  ...
  req.on('response', function(response){
    ...
    response.on('end', function() {
      var regexp = /<UploadId>(.+)<\/UploadId>/g;

      fn(null, regexp.exec(xml)[1]);
    });
  });
  req.end();
};
...
Client.prototype.completeUpload = function(filename, uploadId, parts, headers, fn){
  ...
  req.on('response', function(response){
    if(response.statusCode === 200) return fn(null);
 ...
 * @api public
 */
Client.prototype.putPart = function(filename, buffer, partNumber, uploadId, headers, fn) {
 ...
 if ('function' == typeof headers) {
  ...
  var req = this.request('PUT', filename, headers);
 ...
 * @api public
 */
Client.prototype.putPartStream = function(filename, stream, size, partNumber, uploadId, headers, fn) {
  filename += '?partNumber=' + partNumber;
  filename += '&uploadId=' + uploadId;
  ...
  var req = this.request('PUT', filename, utils.merge({
      'Content-Length': size
    , 'Expect' : '100-continue'
  }, headers));
